...
def ranked_candidates(results: Mapping[str, object], limit: int) -> list[tuple[str, float]]:
    """Read bounded candidate IDs/scores; vector text and metadata are not authority."""
    ids = results.get("ids") or []
    distances = results.get("distances") or []
    if not isinstance(ids, list) or not isinstance(distances, list) or not ids or not distances:
        return []
