...
class ThinkingDirectiveFilterTests(unittest.TestCase):
    ...
    def test_handles_directive_split_across_chunks(self):
        filter_ = ThinkingDirectiveFilter()

        visible_a, directives_a = filter_.push("Remember this. <memory_")
        visible_b, directives_b = filter_.push('write>{"content":"likes tea","importance":3}</memory_write> Done.')
        visible_c, directives_c = filter_.flush()

        self.assertEqual(visible_a + visible_b + visible_c, "Remember this.  Done.")
        self.assertEqual(directives_a, [])
        directives = directives_b + directives_c
        self.assertEqual(len(directives), 1)
        self.assertEqual(directives[0].kind, "memory_write")
        self.assertEqual(directives[0].payload["content"], "likes tea")
        self.assertEqual(directives[0].payload["importance"], 3)
